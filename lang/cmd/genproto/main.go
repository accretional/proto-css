// Command genproto compiles the CSS EBNF grammar into a protobuf schema and the
...
// Outputs:
//
//	proto/css.proto              bundled .proto (human-readable)
...
// ebnfFiles is the concatenation order of the CSS grammar (same as the gallery
// generator). css.ebnf first so CssStyleSheet is the conceptual root.
...
func main() {
	...
	bundledOut := flag.String("bundled", "proto/css.proto", "bundled .proto output")
	...
	// body. The CSS grammar comments heavily inside alternations
	// (PseudoClass, PseudoElement, many property value rules), so feeding the
	// raw source would lose those rules and orphan everything they reference.
	// Each comment is replaced by a space so it can't fuse adjacent tokens.
	// The grammar files keep their comments; only the parser input is cleaned.
	...
	if deps := fdp.GetDependency(); len(deps) > 0 {
		fmt.Printf("note: css.proto imports %v\n", deps)
	}

	// Write the audit reports (pruned rules + dangling refs) for inspection.
	writeReport("proto/GENPROTO_PRUNED.txt", "rules unreachable from CssStyleSheet (pruned)", prunedRules)
...
// protoc both fail to resolve the type. Returns "Parent.field -> .css.Type"
...
// keyword.ebnf/symbol.ebnf turned keyword nonterminals into inline terminals,
// and the compiler names a terminal field after its literal without
// deduplicating; a message with two of the same terminal (e.g. the commas in
// device-cmyk(c,m,y,k)) ends up with duplicate field names that descriptor
// serialization and protoc both reject. The renderer/parser address fields by
// position, so renaming is loss-free. Returns the count of renamed fields.
