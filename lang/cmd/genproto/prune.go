...
// following nonterminal references. The full CSS grammar (~3900 rules) carries
// a long tail of low-level primitives (digit/hex-digit/ident-char character
// ranges) that, after the leaf types are scalarized, no reachable rule
// references anymore. Removing them shrinks the emitted schema from thousands
// of messages to the reachable core and — because the surviving range-bearing
// rules are exactly those orphans — drops the unicode/utf_8.proto import,
// leaving css.proto self-contained.
//
// Scalarizing a PascalCase leaf wrapper (e.g. TimeType) also severs its
// reference to the snake_case atom (time_type), so prune then removes the atom,
// dissolving the name collision the two would otherwise share.
//
