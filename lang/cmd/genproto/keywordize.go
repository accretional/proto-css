...
// literal (a pure keyword or punctuation rule, e.g. `row = "row"`,
// `colon_symbol = ":"`, `double_colon_symbol = "::"`). It unwraps a single
// enclosing sequence or group. Returns the literal and true on match.
...
// so it lowers to an empty marker message (e.g. `message Row {}`). The rule's
// literal is recorded in the prefix map during the prefix pass, so the renderer
// re-emits "row" when it reaches a Row, and a caller selects the keyword by
// instantiating the empty `&Row{}` rather than building a nested keyword
// wrapper. This mirrors proto-sqlite's keyword-marker convention and removes
// the ~1700 redundant *Keyword wrapper messages the compiler would otherwise
// dedup into existence.
//
