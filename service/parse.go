...
func Parse(input string) (*csspb.CssStyleSheet, error) {
	mt, err := protoregistry.GlobalTypes.FindMessageByName("css.CssStyleSheet")
	if err != nil {
		return nil, fmt.Errorf("css.CssStyleSheet not registered: %w", err)
	}
	msg := mt.New()
	...
	sheet, ok := msg.Interface().(*csspb.CssStyleSheet)
	if !ok {
		return nil, fmt.Errorf("internal: parsed %T", msg.Interface())
	}
	return sheet, nil
}
